...
const Carousel = (props) => {
  ...
  const loadCarousel = () => {
    return (
      ...
      {
        //origin input ({large_url, url, id}, index) //large_url
        imgList.map(({url, id}, index) => {
          ...
          return (
            <li key={id} className={'imgWrap ' + (dn ? 'dn': '')} style={{...styles, position: 'absolute', transition: `all ${durationRef.current}ms linear `}} onClick={(e) => { changeCenter({e, index, url})} }>
